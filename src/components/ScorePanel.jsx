import { Trophy, Skull, Crown, Swords, Eye, AlertTriangle, Zap, Layers2, EyeOff, Axe, FlaskConical } from 'lucide-react';
 ...
 */
export function ScorePanel({ score, isReadOnly, showUndead, showMana, showElite, showDouble, showInvisible, showNecromancer }) {
  if (!score) return null;

  const optCount = (showUndead ? 1 : 0) + (showMana ? 1 : 0) + (showElite ? 1 : 0) + (showDouble ? 1 : 0) + (showInvisible ? 1 : 0) + (showNecromancer ? 1 : 0);
  const gridCols = optCount === 0 ? 'grid-cols-5'
    : optCount === 1 ? 'grid-cols-3'
    : optCount >= 6 ? 'grid-cols-6'
      ...
      <div className="bg-gradient-to-br from-dungeon-stone to-dungeon-dark rounded-xl border-2 border-dungeon-gold/50 shadow-2xl overflow-hidden">
        ...
        <div className="p-3 md:p-6">
          ...
          <div className={`hidden md:grid gap-4 ${gridCols}`}>
            ...
            <StatCard icon={<Axe className="text-sky-600" size={32} />} label="Monstres" value={score.monstersDefeated} color="text-sky-400" subtext="+1 pt/monstre" />
            {showUndead && <StatCard icon={<CrossedBonesIcon className="text-yellow-400" size={32} />} label="Morts" value={score.undeadDefeated} color="text-yellow-300" subtext="+1 pt/mort" labelClassName="text-xs" />}
            ...
            {showNecromancer && <StatCard icon={<Skull className="text-green-600" size={32} />} label="Nécro" value={score.necromancersDefeated ?? 0} color="text-green-500" subtext="+1 pt si vaincu" labelClassName="text-xs" />}
            ...
            {showMana && <StatCard icon={<FlaskConical className="text-blue-400" size={32} />} label="Potions" value={score.manaPotionsEarned} color="text-blue-400" subtext="+0 pts" labelClassName="text-xs" />}
          </div>
        </div>
      </div>
    </div>
